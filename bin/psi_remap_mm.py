...
tvec_tmp = tvec_centers_input
tvec_tmp[0]  = (tvec_centers_input[0]  + tvec_centers_input[1] )*0.25
tvec_tmp[-1] = (tvec_centers_input[-1] + tvec_centers_input[-2])*0.25
...
tvec_edges_output[0]    = tvec_centers_output[0]  #- (tvec_centers_output[1]  - tvec_centers_output[0])
tvec_edges_output[-1]   = tvec_centers_output[-1] #+ (tvec_centers_output[-1] - tvec_centers_output[-2])
...
pvec_edges_output[0]    = pvec_centers_output[0]  #- (pvec_centers_output[1]  - pvec_centers_output[0])
pvec_edges_output[-1]   = pvec_centers_output[-1] #+ (pvec_centers_output[-1] - pvec_centers_output[-2])

# Remesh the map:
output_data = downsample_grid.downsamp_reg_grid(input_data, tvec_edges_input,  pvec_edges_input, \
                                                            tvec_edges_output, pvec_edges_output)#, \
#                                                            da=da_input_map)
# Write out result:
